...
var tooltip = d3.select(".vizualizacija").append("div")
  ...
  .style("background-color" , "white");
   ...
   // Create a function to generate the "stroke-dasharray" values
   function getDashArray(totalLength, dashLength, gapLength) {
    const patternLength = dashLength + gapLength; // Use the provided arguments
  ...
  function drawLine(lineElement, dashLength, gapLength, data) {
    ...
    lineElement
      ...
      .attr("stroke-dashoffset", totalLength) // Hide the line initially
      .transition()
      .duration(2000) // Adjust the duration as needed
      .attr("stroke-dashoffset", 0); // Set dash offset to 0 to reveal the line
  }
...
function setStopaRizikaOdSiromastvaPremaStupnjuObrazovanja2022(){
 ...
 //legenda//////////
 svg.append("text")
 .attr("class", "x-axis-label") // You can define a class for styling
 .attr("x", width - 50) // Position it horizontally, you might need to adjust the value
 .attr("y", 50) // Position it vertically, you might need to adjust the value
 .style("text-anchor", "middle") // Center the text horizontally
 .text("MUŠKARCI")
 .attr("fill" , "BLUE"); // The text you want to display

 svg.append("text")
 .attr("class", "x-axis-label") // You can define a class for styling
 .attr("x", width - 50) // Position it horizontally, you might need to adjust the value
 .attr("y", 65) // Position it vertically, you might need to adjust the value
 .style("text-anchor", "middle") // Center the text horizontally
 .text("UKUPNO")
 .style("fill" , "BLACK"); // The text you want to display

 svg.append("text")
 .attr("class", "x-axis-label") // You can define a class for styling
 .attr("x", width - 50) // Position it horizontally, you might need to adjust the value
 .attr("y", 80) // Position it vertically, you might need to adjust the value
 .style("text-anchor", "middle") // Center the text horizontally
 .text("ŽENE")
 .style("fill" , "PINK"); // The text you want to display
 ...
 var barchart = svg.selectAll("rect")
  ...
  .on("mousemove", function(d) { // Add mousemove event
    d3.select(this).style("opacity", 0.7);
    tooltip.html(`Stopa: ${d.Stopa}`)
      .style("left", (d3.event.pageX + 10) + "px")
...
// Transition to gradually increase the height from 0
barchart.transition()
  .delay(function(d, i) { return i * 200; }) // Delay each bar's transition by a different amount
  .duration(1000) // Transition duration in milliseconds
  .attr("y", function(d, i) { return y(d.Stopa); }) // Final Y position
  .attr("height", function(d) { return height - y(d.Stopa); })
     // Final height
...
function setViz2(){
  ...
  naslov.innerHTML= "Voća Hrvatske!"
  var data = [
  ...
  .on("mouseover", function(d, i) {
    d3.select(this).select("path").attr("opacity", 0.7); // Highlight the arc
    tooltip.transition()
      .duration(200)
      .style("opacity", 0.9);
    tooltip.html(data[i].value)
  ...
  .on("mouseout", function() {
    d3.select(this).select("path").attr("opacity", 1); // Restore the arc's opacity
    tooltip.transition()
      .duration(500)
      .style("opacity", 0);
  });
...
function setPragRizikaOdSiromastva(){
  ...
  svg.append("text")
  .attr("class", "x-axis-label") // You can define a class for styling
  .attr("x", width - 100) // Position it horizontally, you might need to adjust the value
  .attr("y", 105) // Position it vertically, you might need to adjust the value
  .style("text-anchor", "middle") // Center the text horizontally
  ...
  .transition()
  .duration(3000)
  .style("opacity", 1); // The text you want to display

  svg.append("text")
  .attr("class", "x-axis-label") // You can define a class for styling
  .attr("x", width - 120) // Position it horizontally, you might need to adjust the value
  .attr("y", 420) // Position it vertically, you might need to adjust the value
  .style("text-anchor", "middle") // Center the text horizontally
 ...
 .tickFormat(function(d, i) { return jednoclanoKucanstvo[i].godina; });
   ...
   svg.append("text")
  .attr("class", "x-axis-label") // You can define a class for styling
  .attr("x", width / 2) // Position it horizontally, you might need to adjust the value
  .attr("y", height + margin.bottom - 10) // Position it vertically, you might need to adjust the value
  .style("text-anchor", "middle") // Center the text horizontally
  .text("Godina")
  .style("font-size", "25px"); // The text you want to display
   ...
   svg.append("text")
  .attr("class", "y-axis-label") // You can define a class for styling
  .attr("transform", "rotate(-90)") // Rotate the text 90 degrees counter-clockwise
  .attr("x", -height / 2) // Position it horizontally, you might need to adjust the value
  .attr("y", -margin.left/2 -10) // Position it vertically, you might need to adjust the value
  .style("text-anchor", "middle") // Center the text horizontally
  ...
  .style("font-size", "25px"); // The text you want to display
   ...
   .attr("class", "cardinal")
   .attr("d", valueline1(jednoclanoKucanstvo))
   .style("stroke", "red")
   .attr("fill", "none");
*/
  ...
  // Draw each line with a gradual effect
  //drawLine(linechart, 30, 10, jednoclanoKucanstvo);
...
function removeActiveSvgElements() {
  ...
  // Iterate through the SVG elements
...
function setStopaRizikaOdSiromastvaPremaDobiISpolu(){
  ...
  .style("text-anchor", "middle") // Center the text horizontally
  .text("ŽENE")
  .attr("fill" , "#A33058"); // The text you want to display
  ...
  .style("text-anchor", "middle") // Center the text horizontally
  .text("UKUPNO")
  .style("fill" , "RED"); // The text you want to display
  ...
  .style("text-anchor", "middle") // Center the text horizontally
  .text("MUŠKARCI")
  .style("fill" , "BLUE"); // The text you want to display
 ...
 .tickFormat(function(d, i) { return stopaRizikaOdSiromastvaSveDobiZenski[i].godina; });
  ...
  .attr("y", height + margin.bottom - 10) // Position it vertically, you might need to adjust the value
  .style("text-anchor", "middle") // Center the text horizontally
  .text("Godina"); // The text you want to display
  ...
  .attr("transform", "rotate(-90)") // Rotate the text 90 degrees counter-clockwise
  .attr("x", -height / 2) // Position it horizontally, you might need to adjust the value
  .attr("y", -margin.left/2) // Position it vertically, you might need to adjust the value
  .style("text-anchor", "middle") // Center the text horizontally
  .text("Postotak %")
  .style("font-size", "25px"); // The text you want to display
...
function setStopaRizikaOdSiromastvaPremaAktivnosti(){
  ...
  // Define an array of line colors
  var lineColors = ["red", "blue", "green", "orange", "purple", "#8C6E0B", "magenta"];
  
    
  
  // Create and style the lines in a loop
  for (var i = 0; i < 7; i++) {
    ...
    var linechart = svg.append("path")
      ...
      .style("stroke", lineColors[i]) // Set the line color
      .attr("fill", "none");
      ...
      svg.append("text")
      .attr("class", "x-axis-label") // You can define a class for styling
      .attr("x", width + margin.right/2) // Position it horizontally, you might need to adjust the value
      .attr("y", 100 + i*35) // Position it vertically, you might need to adjust the value
      .style("text-anchor", "middle") // Center the text horizontally
      .text(legendaOpis[i])
...
function setStopraRizikaOdSiromastvaPremaIntenzitetuBezDjece(){
  ...
  naslov.innerHTML= "Stopa Rizika Od Siromaštva Prema Intenzitetu u 2022. godini";
    ...
    svg.append("text") // Rotate the text 90 degrees counter-clockwise
    .attr("x", width/2) // Position it horizontally, you might need to adjust the value
    .attr("y", 50)
    .style("opacity",0) // Position it vertically, you might need to adjust the value
    .style("text-anchor", "middle") // Center the text horizontally
    ...
    .attr("d", arc)
    .transition() // Apply a transition
    .duration(2000) // Set the duration of the animation (adjust as needed)
    .attrTween("d", function(d) {
      var interpolate = d3.interpolate({ startAngle: 0, endAngle: 0 }, d);
      return function(t) {
        return arc(interpolate(t));
